...
이를 해결하고자 하둡에서 k-means clustering을 통해 군집화 작업을 실시하였다. K-means Clustering을 통해서 data outlier 끼리도 묶었다. 이후 QGIS를 통한 시각화를 통해 이를 확인하였고 data outlier 끼리 묶인 cluster을 제거했다.

[제거된 데이터 이상치]

![errsolve](./img/ex5.png)

이상의 과정을 통해 오전, 오후 시간대별로 오전 4개의 Cluster, 오후 5개의 Cluster로 이루어진 데이터를 얻을 수 있었고, 이를 QGIS를 통해 시각화한 결과는 다음과 같다.
...
***
### [시각화된 자료를 통한 제주도내 주차장 부지 추천]

앞선 작업으로 오후 시간대에 차량의 이동이 잦다는 것을 확인했으므로 오후 시간대의 열지도와 제주도내 기존 주차장 데이터 심볼을 한눈에 확인하면 다음과 같다.

![both](./img/f1.png)

확인한 결과 렌터카의 체류 밀도가 높지만 기존의 주차장이 없는 곳을 표시하면 다음과 같다.

![num](./img/숫자.png)

각 숫자에 대응하는 곳을 VWorld Satelite 레이어를 활용해 위성사진으로 확인한 결과는 다음과 같다.
...
각각의 위치에 대하여 QGIS의 Coordinate Conversion 기능을 활용하여 좌표를 계산한 결과는 다음과 같다.

```
n   longitude   latitude
1   126.311705   33.46018878
2   126.3051754   33.34958932
3   126.16829   33.29886074
4   126.302733   33.28500619
5   126.3664055   33.2322122
6   126.677464   33.43723961
7   126.7520562   33.41296045
8   126.6851903   33.394896
9   126.7807555   33.30802035
```

Google Map을 활용하여 좌표값을 실제 주소로 찍어본 결과는 다음과 같다.
```
1. 제주특별자치도 제주시 애월읍 애월리 2454-2
2. 제주특별자치도 제주시 한림읍 금악리 1242-14
3. 제주특별자치도 제주시 한경면 고산리 3674-3
4. 제주특별자치도 서귀포시 서광남로 82번길
5. 제주특별자치도 서귀포시 안덕면 창천리 786
6. 제주특별자치도 제주시 조천읍 비자림로 661
7. 제주특별자치도 서귀포시 표선면 성읍리 3178-14
8. 제주특별자치도 서귀포시 표선면 붉은오름
9. 제주특별자치도 서귀포시 표선면 토산중앙로 6-1
```

***
### [Cluster Centroids를 통한 제주도내 주차장 부지 추천]

각 군집의 중심값을 활용하여 주차장 부지를 추천할 수 있다. 각 군집의 중심값을 찾아보면 다음과 같은데,

![centroids](./img/centroid.png)

이중 데이터 이상치를 제거한 좌표값을 찾으면 다음과 같다.

```
num   longitude   latitude
1   126.5111969   33.49781852
2   126.8482647   33.45692719
3   126.3030512   33.34547062
4   126.5290812   33.26100932
```

이를 활용하여 QGIS상에 심볼로 나타내보면 다음과 같다.

![centroids](./img/c1.png)

기존의 열지도와 함께 나타내고, 위치를 표시하면 다음과 같다.

![centroids](./img/c2.png)
...
Google Map을 활용하여 좌표값을 실제 주소로 찍어본 결과는 다음과 같다.
```
1. 제주특별자치도 제주시 용문로 18길
2. 제주특별자치도 제주시 한림읍 금악리 1556
3. 제주특별자치도 서귀포시 특별자치도, 대청로 34 KR
4. 제주특별자치도 서귀포시 남원읍 원님로 386번길
5. 제주특별자치도 서귀포시 성산읍 오조리 1402
```

그러나 Cluster Centroids를 활용한 좌표 데이터는 군집의 중심값이기 때문에 좌표 그대로 활용하기보단 좌표값을 참조하여 주변 주차장 입지 조건을 살펴봐야한다

***
### [Insight 도출]

시각화된 자료를 통해 도출된 인사이트와 Cluster Centroids를 통해 도출된 인사이트를 결합하면 주차장 건설 추천 부지의 Priority가 가장 높은 곳이 도출된다.

Priority가 가장 높은 두 곳을 도출한 결과는 다음과 같다.

도출결과 1 - 제주특별자치도 제주시 금오름

![num](./img/ff1.png)

도출결과 2 - 제주특별자치도 서귀포시 표선면 붉은오름

![num](./img/ff2.png)

두 곳 모두 렌터카 체류 밀도가 높고, 시각적으로 분석한 결과, Cluster Centroids를 사용한 결과 모두 겹치는 곳이다. 또한 주변에 주차장이 없는 곳이라 Priority가 높다고 볼 수 있다.
